...
export function WorkoutSummary({ log, plan, onClose }: WorkoutSummaryProps) {
    ...
    const shareBaseUrl = import.meta.env.VITE_SHARE_BASE_URL || 'https://fitwizard.app/join';
    ...
    const buildBrandedShareCard = async () => {
        ...
        ctx.fillText('FitWizard', 740, 130);

        ctx.fillStyle = '#ffffff';
        ctx.font = '700 74px sans-serif';
        ctx.fillText('Workout Complete', 96, 180);

        ctx.font = '500 40px sans-serif';
        ctx.fillStyle = 'rgba(255,255,255,0.9)';
        ctx.fillText(log.dayName, 96, 240);

        const rows = [
            `Duration: ${log.duration} min`,
            `Volume: ${formatVolume(log.totalVolume)} ${preferredWeightUnit}`,
            `Sets: ${completedSets}/${totalSets}`,
            `Difficulty: ${getDifficultyLabel()}`,
            `PRs: ${workoutPRs.length}`,
        ];

        ctx.font = '600 40px sans-serif';
        rows.forEach((row, index) => {
            ctx.fillText(row, 96, 370 + index * 90);
        });

        ctx.font = '500 34px sans-serif';
        ctx.fillStyle = 'rgba(255,255,255,0.95)';
        ctx.fillText(shareBaseUrl, 96, 920);

        ctx.font = '500 28px sans-serif';
        ctx.fillStyle = 'rgba(255,255,255,0.8)';
        ctx.fillText('Generated by FitWizard', 96, 970);
            ...
            <main className="container max-w-2xl mx-auto px-4 py-6 space-y-6">
                ...
                <div className="flex gap-4 pt-4">
                    <Button
                        ...
                        className="flex-1"
                        onClick={async () => {
                            const shareTitle = 'Workout Complete!';
                            const shareIntro = 'I just crushed a workout on FitWizard!';
                            const cardBlob = await buildBrandedShareCard();

                            if (cardBlob) {
                                const shareFile = new File([cardBlob], 'fitwizard-workout-card.png', { type: 'image/png' });
